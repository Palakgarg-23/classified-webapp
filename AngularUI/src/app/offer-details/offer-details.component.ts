...
export class OfferDetailsComponent implements OnInit {
  ...
  offer: Offer = new Offer(1,0,0,"status", 0,"Category", "details", new Date(), new Date(), new Date())
  ...
  engageUser() {
    //call the rest api
    let token = localStorage.getItem("token")
    let id = Number(this.route.snapshot.paramMap.get("offerId"))
    let empId = Number(localStorage.getItem("userId"))
    // if (token != null) {
    //   this.configService.engageOffer(token, this.offer.offerId, empId).subscribe((data: messageResponse) => {
